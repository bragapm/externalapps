...
export async function up(knex) {
  ...
  VALUES
    ...
    ('directus_files','format_file',NULL,'select-dropdown','{"choices":[{"text":"shapefile","value":"shapefile"},{"text":"kml","value":"kml"},{"text":"xls","value":"xls"},{"text":"xlsx","value":"xlsx"},{"text":"csv","value":"csv"},{"text":"geojson","value":"geojson"},{"text":"dxf","value":"dxf"},{"text":"gdb","value":"gdb"},{"text":"tif","value":"tif"},{"text":"las/laz","value":"las/laz"}]}',NULL,NULL,FALSE,FALSE,NULL,'full',NULL,NULL,'[{"name":"Require if folder is Layer Data","rule":{"_and":[{"folder":{"_eq":"${LAYER_DATA_FOLDER_ID}"}}]},"required":true,"options":{"allowOther":false,"allowNone":false}},{"name":"Hide if folder is not Layer Data","rule":{"_and":[{"folder":{"_neq":"${LAYER_DATA_FOLDER_ID}"}}]},"hidden":true,"options":{"allowOther":false,"allowNone":false}}]',FALSE,'task_configurations',NULL,NULL),
    ('directus_files','vector_transform_configuration','alias,no-data,group','group-detail',NULL,NULL,NULL,FALSE,FALSE,NULL,'full',NULL,NULL,'[{"name":"Hide when file is not vector","rule":{"_and":[{"format_file":{"_nin":["shapefile","kml","xls","xlsx","csv","geojson","gdb","dxf"]}}]},"options":{"start":"open"},"hidden":true}]',FALSE,'task_configurations',NULL,NULL),
    ('directus_files','table_name',NULL,'input','{"placeholder":"table_name"}',NULL,NULL,FALSE,FALSE,NULL,'full',NULL,'Lowercase alphabet, numeric, and underscore only','[{"name":"Require when file is vector","rule":{"_and":[{"format_file":{"_in":["shapefile","kml","xls","xlsx","csv","geojson","gdb","dxf"]}}]},"required":true,"options":{"font":"sans-serif","trim":false,"masked":false,"clear":false,"slug":false}}]',FALSE,'vector_transform_configuration','{"_and":[{"table_name":{"_regex":"^[a-z0-9_]+$"}}]}',NULL),
    ('directus_files','is_zipped','cast-boolean','boolean','{"label":"True"}',NULL,NULL,FALSE,FALSE,NULL,'full',NULL,NULL,'[{"name":"Require when file is vector","rule":{"_and":[{"format_file":{"_in":["shapefile","kml","xls","xlsx","csv","geojson","gdb","dxf"]}}]},"required":true,"options":{"iconOn":"check_box","iconOff":"check_box_outline_blank","label":"True"}}]',FALSE,'vector_transform_configuration',NULL,NULL),
